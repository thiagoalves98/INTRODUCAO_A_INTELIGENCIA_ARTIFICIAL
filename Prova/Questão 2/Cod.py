...
def Normalization(df):
	...
	traffic_volume = df.pop('traffic_volume')
	...
	df['date_time'] = pd.to_datetime(df['date_time'], errors='raise').dt.hour
	...
	for key in df_values:
		...
		if key == 'traffic_volume':
			normalized_column = df_values[key]
...
def KNN(df):
	...
	df_entrada = df.iloc[:,:9]
	df_saida = df.iloc[:,9:]

	X_train, X_test, Y_train, Y_test = train_test_split(df_entrada,  df_saida, test_size=0.3, random_state=0)
...
def main():
    ...
    df 		= Open_DataFrame('Metro_Interstate_Traffic_Volume.csv')
    df_nor = Normalization(df)
    df_cor = Correlation(df_nor, 0.80)
    df_out = Outliers(df_cor, 1.5)

    acuracia, pred, test = KNN(df_out)
